...
class NeuralNetworks:
    def __init__(self, hidden_dim,glove_model,tag_to_ix,device,unique_words,second_model,ix_to_tag,val_sen,val_tags ,model_type):
        ...
        self.model_dict = {"FFNN": NeuralNetworks.FFNN_Residual, "LSTM": NeuralNetworks.LSTMLayerNorm, "COMP": NeuralNetworks.LSTMLayerNormResidual}
        self.model_type = model_type
        # Adjust embedding dimension based on model type
        if self.model_type == "COMP":
            self.embedding_dim = glove_model.vector_size + second_model.vector_size + len(PATTERNS)  # Include pattern features
        else:
    ...
    def train(self, train_sentences, train_tags,model_type,lr =0.008,num_epochs=10):
        self.model = self.model_dict[model_type](self.vocab_size, self.embedding_dim, self.hidden_dim, self.embeddings).to(self.device)
        tag_counts = {tag: sum(1 for sublist in train_tags for t in sublist if t == tag) for tag in self.tag_to_ix.keys()}
        ...
        for epoch in range(num_epochs):
            ...
            for sentence, tags in zip(train_sentences, train_tags):
                ...
                tags_tensor = torch.tensor([self.tag_to_ix[tag] for tag in tags], dtype=torch.float).unsqueeze(-1).to(self.device)
    ...
    def predict(self, test_sentences, test_tags=None, verbose=False):
        ...
        if verbose and test_tags is not None:
            ...
            return [[self.ix_to_tag[ix] for ix in seq] for seq in all_predictions]
    ...
    class LSTMLayerNormResidual(nn.Module):
        def __init__(self, vocab_size, embedding_dim, hidden_dim, embeddings):
            ...
            self.shortcut = nn.Linear(hidden_dim * 2, hidden_dim // 2)  # Shortcut should match the last layer's input dimension
